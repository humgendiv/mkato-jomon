...
/usr/bin/bcftools mpileup -q 30 -Q 30 -C 50 -Ou -r "chr${chr}"


if [ "$sample_name" = "T5" ]; then
/usr/bin/bcftools mpileup -q 30 -Q 30 -C 50 -Ou -r "chr${chr}" -f "${DATADIR}reference/hg19.fa" "$DATADIR/bam/share/T5_p1_p2_p3.bam" | bcftools call -c -V indels |
...
/usr/bin/bcftools mpileup -q 30 -Q 30 -C 50 -Ou -r "chr${chr}" -f "${DATADIR}reference/hg19.fa" "$DATADIR/bam/share/I4_filtered_RG_dedup_trim.bam" | bcftools call -c -V indels |
/home/mkato/Repo/msmc/msmc-tools/bamCaller.py $depth "${OUTDIR}/${sample_name}_chr${chr}_mask.bed.gz" | gzip -c > "${OUTDIR}/${sample_name}_chr${chr}.vcf.gz"
